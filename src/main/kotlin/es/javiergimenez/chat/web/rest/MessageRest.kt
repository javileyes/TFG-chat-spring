...
class MessageRest {
    ...
    fun getMessages(@PathVariable channelId: String): ResponseEntity<List<Message>> {
        return ResponseEntity.status(HttpStatus.CREATED).body(messageService.findByChannelId(channelId))
    }
    ...
    fun postMessage(@PathVariable channelId: String, @RequestParam body: String): ResponseEntity<Message> {
        return ResponseEntity.status(HttpStatus.CREATED).body(messageService.create(channelId, body, userService.whoAmI().id!!))
    }
